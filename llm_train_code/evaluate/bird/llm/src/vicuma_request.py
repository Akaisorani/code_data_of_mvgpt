...
def do_model_request(model, engine, prompt, max_tokens, temperature, stop):
    """
  ...
  "usage": null
    ...
    """
    params = {}
    # request_dict["model"] = "analyticai-13b-mama_sql_v530"
    # params['model'] = request_dict.get("model")
    # prompt = request_dict.get("prompt")
    if model is None or model == "":
        model = "analyticai-13b-mama_sql_v530"
    params['model'] = model
...
#    return result
def collect_response_from_gpt(db_path_list, question_list, api_key, model, engine, knowledge_list=None):
    '''
    :param db_path: str
    :param question_list: []
    ...
    for i, question in tqdm(enumerate(question_list)):
        ...
        flag, plain_result = do_model_request(model=model, engine=engine, prompt=cur_prompt, max_tokens=256, temperature=0, stop=['--', '\n\n', ';', '#'])
        print("flag is {}, and result is {}".format(flag, plain_result))
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--api_key', type=str, required=False)
    ...
    if args.use_knowledge == 'True':
        responses = collect_response_from_gpt(db_path_list=db_path_list, question_list=question_list, api_key=args.api_key, model=args.model, engine=args.engine, knowledge_list=knowledge_list)
    else:
        responses = collect_response_from_gpt(db_path_list=db_path_list, question_list=question_list, api_key=args.api_key, model=args.model, engine=args.engine, knowledge_list=None)
