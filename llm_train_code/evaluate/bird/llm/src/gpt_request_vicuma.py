...
def do_model_request(engine, prompt, max_tokens, temperature, stop):
    """
  ...
  "usage": null
    ...
    params['model'] = "analyticai-13b-mama_sql_v530"
...
def collect_response_from_gpt(db_path_list, question_list, api_key, engine, knowledge_list=None):
    ...
    for i, question in tqdm(enumerate(question_list)):
        ...
        flag, plain_result = do_model_request(engine=engine, prompt=cur_prompt, max_tokens=256, temperature=0, stop=['--', '\n\n', ';', '#'])

        # flag, plain_result = connect_gpt(engine=engine, prompt=cur_prompt, max_tokens=256, temperature=0, stop=['--', '\n\n', ';', '#'])
        # pdb.set_trace()
