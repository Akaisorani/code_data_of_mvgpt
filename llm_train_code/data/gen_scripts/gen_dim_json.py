...
#extract dim value to json, so as to lookup the value with embeddings from vector db
...
def genPromptAndSql(data, prompt_template1, sql_template1):
    ...
    for i in range(0,dim_list_cnt):
        if prompt_dim[i+1]['header_id'] == 'thedate':
            continue
    ...
    sql = template2.substitute(metric_sum_func=metric_sum_func,table_name=table_name,main_dim_comment=main_dim_comment,other_dim_list=other_dim_list,equal_comment='',main_dim=main_dim,rand_dim_val=rand_dim_val
...
template_list = [{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${main_dim_comment}${equal_comment}${rand_dim_val}, ${time_range_comment}每天的${metric_comment}是多少?${order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${main_dim_comment}${rand_dim_val}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${time_range_comment}每周${main_dim_comment}${equal_comment}${rand_dim_val}${other_dim_list}的${metric_comment}是多少?${week_order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${main_dim_comment}${rand_dim_val}${other_dim_list}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${main_dim_comment}${equal_comment}${rand_dim_val}, ${time_range_comment}每月的${metric_comment}是多少?${month_order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${main_dim_comment}${rand_dim_val}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${main_dim_comment}${equal_comment}${rand_dim_val}${other_dim_list}, ${time_range_comment}每季度的${metric_comment}是多少?${quarter_order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${main_dim_comment}${rand_dim_val}${other_dim_list}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${time_range_comment}${main_dim_comment}${equal_comment}${rand_dim_val}${other_dim_list}每年${metric_comment}是多少?${year_order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${main_dim_comment}${rand_dim_val}${other_dim_list}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${other_dim_list}, ${time_range_comment}的${metric_comment}是多少?按${main_dim_comment}分组 ${dim_order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${other_dim_list}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${main_dim_comment}${equal_comment}${rand_dim_val}${other_dim_list}, ${time_range_comment}${metric_comment}是多少?${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${main_dim_comment}${rand_dim_val}${other_dim_list}\"}"}

,{"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下: ${other_dim_list}, ${time_range_comment}每天的${metric_comment}是多少?按${main_dim_comment}分组${order_prompt}.${limit_prompt}',
"sql_template":"{\"dim_value_list\":\"${other_dim_list}\"}"}

, {"prompt_template":'从下面问题中提取维度值,返回结果为json。问题如下:${other_dim_list}, ${time_range_comment}${top_comment}的${main_dim_comment}${top_question_mark}',
"sql_template":"{\"dim_value_list\":\"${other_dim_list}\"}"}
]

# template_list = [
#   {"prompt_template":'表名称: ${table_name}\n 描述:商家数据 \ntable columns: ${col_fileds}。\n使用以上表从下面问题中提取维度值,返回结果为json。问题如下:  ${time_range_comment}${metric_comment}是多少? ${one_metric_order_comment}.${limit_prompt}',
...
def genrate_corpus(rows, table_schema):
    ...
    with open("json/corpus_dim_json.json", "w") as f:
