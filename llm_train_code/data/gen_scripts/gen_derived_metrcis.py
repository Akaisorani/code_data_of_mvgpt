...
#add an extra derived columns in prompt, so that user can ask questions about derived metrcis like roi, cvr
...
def genPromptAndSql(data, prompt_template1, sql_template1):
    ...
    derived_metric_field1_name = getRandomStr(tmp_dim_field_len)
    derived_metric_field1_desc = getRandomStr(tmp_dim_field_len)
    if random.randint(0, 2) != 1:
        derived_metric_field1_desc = derived_metric_field1_name
    derived_metric_field1_expr = getRandomStr(tmp_dim_field_len) + getRandomStr(random.randint(3, 10))
    derived_metric_field2_name = getRandomStr(random.randint(3, 10))
    derived_metric_field2_desc = getRandomStr(tmp_dim_field_len) +","+ getRandomStr(random.randint(3, 10))
    if random.randint(0, 1) == 1:
        derived_metric_field2_desc += "," + derived_metric_field2_name
    derived_metric_field2_expr = getRandomStr(tmp_dim_field_len) + getRandomStr(random.randint(3, 10))

    tmp_derived_metric_func = ["sum",'count','avg','max','min']
    tmp_derived_metric_func_op = ["/",'+','-','*']
    if random.randint(0, 1) == 1:
        derived_metric_field1_expr = random.sample(tmp_derived_metric_func, 1)[0] + '('+derived_metric_field1_expr+")"+random.sample(tmp_derived_metric_func_op, 1)[0]+random.sample(tmp_derived_metric_func, 1)[0]+"("+ getRandomStr(random.randint(3, 10)) +")"
    
    tmp_derived_opt2 = random.randint(0, 1) 
    if tmp_derived_opt2 == 1:
        derived_metric_field2_expr = random.sample(tmp_derived_metric_func, 1)[0] + '('+derived_metric_field2_expr+")"+random.sample(tmp_derived_metric_func_op, 1)[0]+random.sample(tmp_derived_metric_func, 1)[0]+"("+ getRandomStr(random.randint(3, 10)) +")"

    derived_metric_field1 = { 'header_id':derived_metric_field1_name,
                   'header_type': derived_metric_field1_expr
                   , "header_name": derived_metric_field1_desc
                   ,'derived':"{derived_mtric}"
                 }
    derived_metric_field2 = { 'header_id':derived_metric_field2_name,
                'header_type': derived_metric_field2_expr
                , "header_name": derived_metric_field2_desc
                ,'derived':"{derived_mtric}"
                }
    ...
    prompt_metric.append(derived_metric_field1)
    prompt_metric.append(derived_metric_field2)
    prompt_metric = random.sample(prompt_metric, 12)
    ...
    for item in all_fields:
        ...
        col_fileds = col_fileds + item['header_id'] +"[" + item['header_type']  +"]"
        if 'derived' in item.keys():
            col_fileds += item['derived']
        col_fileds += "(" +  item['header_name'] + ")"
    
    #add derived metric to a seprate section in prompt
    #col_fileds += "\n derived metrics:"
    #if random.randint(0, 1) == 1:
        #add a random one, not used
    #    col_fileds += "\n"+getRandomStr(random.randint(6, 10))+"------"+getRandomStr(random.randint(6, 10))+ "------" + getRandomStr(random.randint(6, 10)) 
    #col_fileds += "\n"+derived_metric_field1['header_id']+"------"+derived_metric_field1['header_type']+ "------" + derived_metric_field1['header_name'] 
    #col_fileds += "\n"+derived_metric_field2['header_id']+"------"+derived_metric_field2['header_type']+ "------" + derived_metric_field2['header_name'] 
    ...
    one_metric_sum_func = ''
    one_metric_col = ''
    dim_metric_cnt = random.randint(0, 2)
    for i in range(0,dim_metric_cnt):
        if derived_metric_field1_desc == prompt_metric[i]['header_name'] or derived_metric_field2_desc == prompt_metric[i]['header_name']:
            continue
    ...
    dim_derived_metric_cnt = random.randint(1, 2)
    if(len(metric_comment) > 0):
        metric_comment += ","
        metric_sum_func += ","
    metric_comment +=  derived_metric_field1_desc
    metric_sum_func += derived_metric_field1_expr +" as " + derived_metric_field1_name
    if dim_derived_metric_cnt > 1:
        metric_comment += ","+derived_metric_field2_desc.split(",")[random.randint(0, len(derived_metric_field2_desc.split(",")) -1 )]
        metric_sum_func += ","+derived_metric_field2_expr +" as " + derived_metric_field2_name
...
def genrate_corpus(rows, table_schema):
    ...
    with open("json/corpus_derived_metrics.json", "w") as f:
